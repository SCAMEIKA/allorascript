...
echo -e "\nПодписаться на канал SCAMEIKA ➧➧➧➧ https://t.me/scameika_crypto\n"

sleep 2

while true; do
    echo "1. Установка узла Allora"
    echo "2. Проверить логи узла Allora"
    echo "3. Проверить статус узла Allora"
    echo "4. Проверить обновление узла Allora"
...
EOL
            ...
            if curl --location 'http://localhost:6000/api/v1/functions/execute' \
                ...
                echo "Проверка статуса узла: Успешно"
            else
                echo "Проверка статуса узла: Ошибка"
            fi

            echo -e "\nПодписаться на канал SCAMEIKA ➧➧➧➧ https://t.me/scameika_crypto\n"
            ;;
        2)
            echo "Ожидайте, логи скоро появятся. Для выхода из отображения логов нажмите CTRL+C. Пока Вы можете подписаться на канал SCAMEIKA ➧➧➧➧ https://t.me/scameika_crypto\n"
            sleep 60
        ...
        3)
            ...
            ;;
        4)
            echo "Проверка обновления узла..."
            response=$(curl -s http://localhost:8000/update)
            if [ "$response" == "0" ]; then
                echo "Версия узла актуальна."
            else
                echo "Версия узла неактуальна, обновитесь."
            fi
            ;;
        5)
